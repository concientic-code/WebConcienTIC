...
(`docs/concientic_v11_reference.html`).

Antes de tocar código, lee `kiro/specs/concientic-web-redesign.md` (v4), que
...
## J. Subpáginas DQUILIBRIO (nuevo — ver Prompt 08 para el detalle completo)
27. Copia los 4 archivos de `assets/dquilibrio-subpaginas/` (atencion.html,
    informacion.html, discernimiento.html, autonomia.html) tal cual, sin
    modificarlos, a `public/dquilibrio/`.
28. Convierte las 4 tarjetas de la sección DQUILIBRIO en enlaces `<a>` con
    `target="_blank" rel="noopener noreferrer"` hacia esas 4 rutas,
    conservando el estilo `.card` existente y agregando el afordance "Abrir →"
    solo visible al hover/focus.
29. No toques el contenido de las 4 subpáginas (incluye no corregir el logo
    antiguo que tienen embebido) sin confirmación explícita.

## Verificación final
- `npm run build`, lint y typecheck sin errores.
- Compara visualmente contra `docs/concientic_v11_reference.html` sección por
...
- Confirma que las 4 tarjetas de DQUILIBRIO abren sus subpáginas en pestaña
  nueva y que esos 4 archivos no fueron reescritos.
