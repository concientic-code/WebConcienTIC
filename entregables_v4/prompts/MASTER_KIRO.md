# MASTER PROMPT — ConcienTIC Web / Next.js / Vercel (v4, incluye subpáginas DQUILIBRIO)
...
3. `kiro/specs/concientic-web-redesign.md` (v4 — diff completo desde el
   prototipo original v5 hasta el estado final v11: orden de secciones,
   logo, copy del Hero, ilustraciones, sección Equipo, tipografía del
   Manifiesto, interacción/hover, accesibilidad y subpáginas DQUILIBRIO)
4. `.kiro/steering/*`
5. `docs/concientic_v11_reference.html` como referencia visual final (no como
...
5. Implementa las subpáginas DQUILIBRIO como assets estáticos enlazados
   (`prompts/08_SUBPAGINAS_DQUILIBRIO.md`).
6. QA visual, funcional, accesibilidad y performance
...
## SUBPÁGINAS DQUILIBRIO
Ver reglas completas en `concientic-web-redesign.md` (§10) y
`prompts/08_SUBPAGINAS_DQUILIBRIO.md`. En resumen: las 4 tarjetas de la
sección DQUILIBRIO son enlaces (`target="_blank"`) a 4 experiencias ya
diseñadas (`assets/dquilibrio-subpaginas/*.html`) que se copian tal cual a
`public/dquilibrio/`, sin reescribirlas ni "mejorarlas".
